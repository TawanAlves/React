  ...
  <div>
    ...
    >
      <template slot="footer">
        ...
        <a-button type="primary" @click="okModal" v-if="!disabled">确定</a-button>
      </template>
      <div>
        <a-form-model
          ref="ruleForm"
        ...
        >
          <a-form-model-item label="风险点">
            <a-input disabled="true" v-model="form.pointName"></a-input>
          </a-form-model-item>
          <a-form-model-item label="风险类型">
            <a-input disabled="true" v-model="form.pointType"></a-input>
          </a-form-model-item>
          <a-form-model-item label="隐患名称" prop="name">
            <a-input :disabled="disabled" v-model="form.name"></a-input>
          </a-form-model-item>

          <a-form-model-item label="隐患表述">
            <a-textarea :disabled="disabled" v-model="form.description"></a-textarea>
          </a-form-model-item>

          <a-form-model-item label="风险级别" prop="level">
            <a-select :disabled="disabled" v-model="form.level" @change="levelChange">
          ...
          <a-form-model-item label="隐患类别" prop="type">
            <a-select :disabled="disabled" v-model="form.type" @change="typeChange">
          ...
          <a-form-model-item label="隐患地点" prop="address">
            <a-input :disabled="disabled" v-model="form.address"></a-input>
          </a-form-model-item>
          <a-form-model-item label="隐患责任部门" prop="dutyDeptId">
            <department-select :disabled="disabled" :value="form.dutyDeptId" @change="dutyDeptChange" />
          </a-form-model-item>
          ...
          <a-form-model-item label="评估人" prop="evaluatorName">
            <a-row :gutter="24">
              <a-col :span="20">
                <a-input disabled="true" v-model="form.evaluatorName"></a-input>
              </a-col>
              <a-col :span="2">
                <a-button type="primary" size="small" shape="round" @click="selectPerson" v-if="!disabled"> 选择 </a-button>
              </a-col>
            </a-row>
          </a-form-model-item>
        </a-form-model>
  ...
  methods: {
    setFormData(model) {
      ...
      if (model && model.id) {
        ...
        this.form = JSON.parse(JSON.stringify(model))
        this.form = {
          planPointId: model.planPointId, //检查计划关联风险点表
          pointName: model.pointName, //风险点名称
          pointType: model.pointType, //风险类型
    ...
    changeRiskPoint(risk) {
      ...
      let riskobj = risk[0]
      this.form.planPointId = riskobj.id //检查计划关联风险点表
