  ...
  <page-container :title="false">
    <a-card :bordered="false" style="margin: 15px;" :body-style="{'padding-bottom': '0px'}">
      <div class="table-page-search-wrapper">
        <a-form layout="inline">
          <a-row :gutter="24">
            <a-col :lg="6" :md="12" :sm="12" :xs="24">
              <a-form-item label="风险类型">
                <a-select v-model="queryParam.riskTypeId" placeholder="风险类型">
                  <a-select-option v-for="val in riskTypeOptions " :key="val.id">{{ val.dictValue }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :lg="6" :md="12" :sm="12" :xs="24">
              <a-form-item label="风险级别">
                <a-select v-model="queryParam.riskLevelId" placeholder="风险级别">
                  <a-select-option
                    v-for="val in riskLevelOptions"
                    :key="val.riskLevelId"
                  >{{ val.riskLevelName }}</a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :lg="6" :md="12" :sm="12" :xs="24">
              <a-form-item label="设备名称">
                <a-input v-model="queryParam.equipmentName" placeholder="设备名称" />
              </a-form-item>
            </a-col>
            <a-col :lg="6" :md="12" :sm="12" :xs="24">
          ...
          <a-row :gutter="24">
            <a-col :md="24" class="table-page-search-submitButtons">
              <a-space>
    ...
    >
        ...
        <div class="text-right">
          <a-space>
            <a-button
              ...
              icon="plus"
              @click="equipmentAdd"
              v-if="permission.risk_equipment_add"
            >新建</a-button>
            <a-button
              icon="delete"
              :disabled="!hasSelected"
              @click="equipmentRemove"
              v-if="permission.risk_equipment_delete"
            >删除</a-button>
      ...
      <s-table
        ref="equipmentTable"
        size="default"
        :scroll="{y: `calc(100vh - 500px)`, x: 1510}"
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <template>
            ...
            <a-divider
              type="vertical"
              v-if="((record.status === 1 && userInfo.id === record.gmtCreateId) || (record.status !== 0 && permission.risk_equipment_recall))"
            />
            <a
              @click="submitRecord(record, 0)"
              v-if="((record.status === 1 && userInfo.id === record.gmtCreateId) || (record.status !== 0 && permission.risk_equipment_recall))"
            >撤回</a>
            <a
              @click="equipmentEdit(record)"
              v-if="record.status === 0 && (userInfo.id === record.gmtCreateId ||permission.risk_equipment_edit)"
            >编辑</a>
            <a-divider
              type="vertical"
              v-if="record.status === 0 && (userInfo.id === record.gmtCreateId ||permission.risk_equipment_submit)"
            />
            <a
              @click="submitRecord(record, 1)"
              v-if="record.status === 0 && (userInfo.id === record.gmtCreateId || permission.risk_equipment_submit)"
            >提交</a>
            <a-divider
              type="vertical"
              v-if="record.status === 0 && (userInfo.id === record.gmtCreateId || permission.risk_equipment_delete)"
            />
            <a
...
export default {
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        ...
        return this.getList(URLS.equipmentRisk, requestParameters).then(res => {
  ...
  methods: {
    selectOptionsInit() {
      ...
      this.getDict('RISK_TYPE').then(({ data, success }) => {
        if (success) {
    ...
    removeConfirm(ids) {
      ...
      this.$confirm({
        ...
        cancelText: '取消',
        onOk() {
          _this.deleteByIds(URLS.equipment, ids).then(res => {
            if (res.code === 200) {
    ...
    loadRowData(id, callback) {
      this.getById(URLS.equipment, id).then(({ data, success }) => {
        if (success) {
          if (data.approveId === -1) {
    ...
    equipmentOk({ formData }) {
      ...
      this.submitForm(URLS.equipment, formData, method, {
    ...
    updateStatus(ids, status) {
      this.submitForm(URLS.updateEquipmentStatus, ids, 'put', { params: { status } })
        .then(({ success }) => {
          if (success) {
