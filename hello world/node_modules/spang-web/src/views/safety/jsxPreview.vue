...
const layouts = {
  colFormItem: function (conf, h, ctx, isList = false) {
    ...
    const item = <el-col span={conf.span}>
                  <el-form-item
                  label-width={labelWidth}
                  label={isList ? '' : conf.label}
                  prop={conf.columnName}>
                    <render
  ...
  rowFormItem: function (conf, h, ctx) {
    ...
    const row = <el-col span={conf.span || 24}>
                <el-row {...{ props: props }}>
  ...
  methods: {
    submitForm () {
      ...
      this.$refs[this.confGlobal.formRef].validate(valid => {
        ...
        // TODO 提交表单
        var formData = this[this.confGlobal.formModel]
        var fields = this.confGlobal.fields
        var fieldsData = {}
        for (var key in formData) {
          var item = fields.find(item => item.columnName === key)
          var data = {
            tableName: item.tableName,
            isCondition: item.isCondition,
            columnValue: formData[key]
          }
          fieldsData[key] = data
        }
        var param = {
          processId: this.confGlobal.processId,
          fields: fieldsData
        }
        param.fields.workType.columnValue = 1
        param.fields.executePeriod.columnValue = 2
        console.log('!!!!!!!!!', param)
        workPlan.startProcessTask(param).then(res => {
          this.resetForm()
