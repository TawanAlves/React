<template>
  <a-card :bordered="false" style="margin: 15px;">
    <a-form-model :model="form" ref="ruleForm" :rules="rules" :label-col="{span: 7}" :wrapper-col="{span:10}">
      <a-row class="form-title">
        <h3 >{{ formTitle }}</h3>
      </a-row>
      <a-form-model-item label="作业证编号" v-if="id !== '-1'">
        <a-input
          v-model="form.code"
          disabled/>
      </a-form-model-item>
      <a-form-model-item label="申请单位">
        <a-input
          v-model="form.applicantUnitName"
          disabled/>
      </a-form-model-item>
      <a-form-model-item label="申请人">
        <a-input
          v-model="form.applicantName"
          disabled/>
      </a-form-model-item>
      <a-form-model-item label="单位类型" prop="constructionUnitType">
        <a-radio-group v-model="form.constructionUnitType">
          <a-radio :value="0">
            内部单位
          </a-radio>
          <a-radio :value="1">
            外来施工单位
          </a-radio>
        </a-radio-group>
      </a-form-model-item>
      <a-form-model-item label="作业单位" prop="workUnitId">
        <department-select v-model="form.workUnitId" @change="workUnitChange" placeholder="请选择作业单位"></department-select>
      </a-form-model-item>
      <a-form-model-item label="施工单位" prop="field6">
        <a-input
          placeholder="请输入施工单位"
          v-model="form.field6"
        />
      </a-form-model-item>
      <a-form-model-item label="监护人" prop="localeGuardianId">
        <person-list-select
          v-model="form.localeGuardianId"
          @change="proctorChange"
          :deptId="form.workApplyCompanyId"
          placeholder="请选择监护人"
        />
      </a-form-model-item>
      <a-form-model-item label="作业项目" prop="field2">
        <a-input
          placeholder="请输入作业项目"
          v-model="form.field2"
        />
      </a-form-model-item>
      <a-form-model-item label="作业地点" prop="workAddress">
        <a-input
          placeholder="请输入作业地点"
          v-model="form.workAddress"
        />
      </a-form-model-item>
      <a-form-model-item label="作业内容" prop="workContent">
        <a-input
          placeholder="请输入作业内容"
          v-model="form.workContent"
        />
      </a-form-model-item>
      <a-form-model-item label="风险分析" prop="field1">
        <a-input
          placeholder="请输入风险分析"
          v-model="form.field1"
        />
      </a-form-model-item>
      <a-form-model-item label="申请作业起止时间" prop="planStartEndTime">
        <a-range-picker
          @change="onPlanTimeChange"
          v-model="form.planStartEndTime"
          valueFormat="YYYY-MM-DD HH:mm:ss"
          showTime
          style="width: 100%"
        />
      </a-form-model-item>
      <div style="text-align: center; margin-top: 10px;">
        <a-button type="primary" @click="submitForm" style="width: 122px;">提交</a-button>
        <a-button type="second" @click="goBack" style="width: 122px;margin-left: 20px">返回</a-button>
      </div>
    </a-form-model>
  </a-card>
</template>

<script>
import { mapGetters } from 'vuex'
import { addHotwork, getHotworkDetail } from '@/api/hot-work'
import DepartmentSelect from '../../components/DepartmentSelect'
import PersonListSelect from '../../components/PersonListSelect'
export default {
  name: 'AddTikcet',
  components: { DepartmentSelect, PersonListSelect },
  props: {
    id: {
      type: String,
      required: true,
      default: '-1'
    }
  },
  data () {
    return {
      form: {
        workUnitName: null,
        workUnitId: null,
        applicantUnitName: null,
        workApplyCompanyId: null,
        applicantName: null,
        workApplyUserId: null,
        planStartTime: null,
        planEndTime: null,
        constructionUnitType: 0,
        localeGuardianId: null,
        localeGuardianName: null,
        planStartEndTime: null
      },
      rules: {
        workAddress: [
          { required: true, message: '请输入作业地点', trigger: 'blur' }
        ],
        field1: [{ required: true, message: '请输入风险分析', trigger: 'blur' }],
        field2: [{ required: true, message: '请输入作业项目', trigger: 'blur' }],
        field6: [{ required: true, message: '请输入施工单位', trigger: 'blur' }],
        localeGuardianId: [
          { required: true, message: '请选择监护人', trigger: 'blur' }
        ],
        workContent: [{ required: true, message: '请输入主要作业内容', trigger: 'blur' }],
        planStartEndTime: [
          { required: true, message: '请选择计划作业起止时间', trigger: 'change' }
        ],
        workUnitId: [{ required: true, message: '请选择作业单位', trigger: 'change' }]
      }
    }
  },
  computed: {
    ...mapGetters(['userInfo']),
    formTitle () {
      if (this.id === '-1') {
        return '新建检修作业票'
      } else {
        return '修改检修作业票'
      }
    }
  },
  methods: {
    onPlanTimeChange (date) {
      this.form.planStartTime = date[0]
      this.form.planEndTime = date[1]
      this.form.planStartEndTime = date
    },
    filterOption (input, option) {
      return (
        option.componentOptions.children[0].text.toLowerCase().indexOf(input.toLowerCase()) >= 0
      )
    },
    goBack () {
      this.$router.push('/safety/task/manage')
    },
    submitForm () {
      this.$refs.ruleForm.validate(valid => {
        if (valid) {
          const params = Object.assign({}, this.form)
          delete params.planStartEndTime
          params.status = 2
          params.ticketType = '8';
          // params.localeGuardianId
          let method = 'post';
          if (this.id !== '-1') {
            method = 'put';
          }
          const url = `/spang-safety/basic-information`;
          this.submitFormApi(method,url, params).then(res=>{
            if(res.code === 200){
              this.$notification['success']({
                message: '提示',
                description: '提交成功',
                duration: 8
              })
              this.$router.push('/safety/task/manage');
            }
          })
        } else {
          console.error('error submit!!')
          return false
        }
      })
    },
    loadFormData () {
      this.getList('/spang-safety/basic-information/' + this.id,{}).then(res => {
        if (res.code === 200) {
          this.form = Object.assign({ planStartEndTime: [res.data.planStartTime, res.data.planEndTime] }, res.data)
        }
      })
    },
    proctorChange ({ id, person }) {
      this.form.localeGuardianName = person.realName
    },
    workUnitChange ({ id, label }) {
      this.form.workUnitName = label
    }

  },
  created () {
    if (this.id !== '-1') {
      this.loadFormData()
    } else {
      this.form.applicantUnitName = this.userInfo.deptName
      this.form.applicantUnitId = this.userInfo.deptId
      this.form.applicantName = this.userInfo.realName
      this.form.applicantId = this.userInfo.id
    }
  }
}
</script>

<style scoped lang="less">
  /deep/ .ant-table {
    height: auto;
  }
</style>
