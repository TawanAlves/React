  ...
  <div>
    <a-card title="人员指派" style="margin: 15px;">
      <a-form-model :model="form" ref="ruleForm" :label-col="{span: 7}" :wrapper-col="{span:10}">
        ...
        <a-form-model-item label="可能产生有害物质">
          <a-input
            v-model="form.harmfulSubstances"
            disabled/>
        </a-form-model-item>
...
export default {
  ...
  methods: {
    loadDetail () {
      return this.getByIdRestful('/spang-safety/related-user', this.ticketId)
    },
    getAuth () {
      getUserAuth({
    ...
    changeOtherPerson (users) {
      ...
      if (this.personType === 1) {
        this.form.workHotUserId = users[0].userId
        this.form.workHotUserName = users[0].userName
        this.form.workHotUserSignUrl = ''
      } else {
    ...
    submitChange () {
      ...
      this.submitForm('/spang-safety/related-user', params, 'put').then(({ data, code }) => {
        if (code === 200) {
          this.$notification['success']({
            message: '提示',
            description: '更换成功',
