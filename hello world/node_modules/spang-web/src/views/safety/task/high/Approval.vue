<template>
  <a-card title="审核记录" style="margin: 15px;">
    ...
    <a-divider />
    <horizontal-list :list-data="detailData"></horizontal-list>
  </a-card>
</template>
          ...
          this.historyDatas = data.map(item => (
            {
              ...
              operateName: APPROVE_TYPES[item.operateStatus],
    ...
    getDetail () {
      this.getById('/spang-safety/affirm', this.ticketId).then(({ data, success }) => {
					if (success) {
						data.forEach(item => {
							if (item.userType === 44) {
								this.detailData.sign1 = item.signatureUrl
								this.detailData.time1 = item.signTime
							} else if (item.userType === 45) {
								this.detailData.sign2 = item.signatureUrl
								this.detailData.time2 = item.signTime
							}
						})
					}
				})
