...
export default {
  ...
  methods: {
    async loadDetail () {
        const assignData = await this.getByIdRestful('/spang-safety/related-user', this.ticketId)
        if (assignData.code === 200) {
          this.form = assignData.data
        }
    },
    getAuth () {
      getUserAuth({
          ...
          deptId: this.userInfo.workUnitId
        }).then(({ data, code }) => {
          if (code === 200) {
            this.canSubmit = data
          }
